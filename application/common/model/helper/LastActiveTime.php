...
trait LastActiveTime
{
     ...
     */
    public static function syncUserLastActiveTime()
    {
        // 获取昨天的开始、结束时间戳
        list($begin_time, $end_time) = Time::yesterday();
        // 日期的缓存键名, 例：last_active_at_2019_06_28
        $date_key = self::getHashFormTime($begin_time);
        ...
        foreach ($active_users as $user_id => $active_time) {
            // 会将 `user_1` 转换为 1
            $user_id = str_replace(self::LAST_ACTIVE_PREFIX, '', $user_id);
     ...
     */
    private static function getHashFormTime($time)
    {
        $date_str = date('Y_m_d', $time);
        return self::LAST_ACTIVE_CACHE_KEY.$date_str;
    }
    ...
    public function recordLastActiveTime()
    {
        ...
        $current_time = time();
        // 日期的缓存键名, 例：last_active_at_2019_06_28
        $date_key = self::getHashFormTime($current_time);
        // 已缓存的最后活跃用户列表
     ...
     * @return   string             [description]
     */
    public function getLastActiveKeyAttr()
    {
        return self::LAST_ACTIVE_PREFIX.$this->id;
    }
     ...
     * @return   string             [description]
     */
    public function getLastActiveAtAttr()
    {
        $current_time = time();
