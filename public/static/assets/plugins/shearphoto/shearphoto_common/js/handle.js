...
window.ShearPhoto.MINGGE(function() {
	//██████████重要设置████████████████
	var relativeUrl = "/static/plugins/shearphoto/shearphoto_common"; //你不要在后面加斜杠，系统会自动给你加上斜杠，不信看下面！   index.html的JS引用路径自己改，很简单的说
	/*
	...
	Shear.config({
		...
		/*HTML5重点功能*/
		HTML5MAX: 300, //默认请设0 (最大尺寸做事)， HTML上传截图最大宽度， 宽度越大，HTML5截出来的图片容量越大，服务器压力就大，截图就更清淅！ 设得越小 HTML5截出来的图片容量越小.但是造成一定程序的不清淅，请适量设置 当然开启HTML5切图，该设置才有效
		...
		proportional: [1 / 1, <!--截框的宽高比例（宽除以高的比例值，这个设置其实就是1，对！你可以直接写1  如填3/4 那么就是0.75的比例,不设比例请设为0，注意更改比例后，后端也要进行相应设置，否则系统会给你抱出错误-->
			...
			*/
			200, //必须整数！启动后的截框初始宽度(整数型，禁止含小数点)  

			200 //比例设置后，这个高度无效，由宽和比例来决定(整数型，禁止含小数点)  
		],

		Min: 50, //截框拉伸或拖拽不能少于多少PX(整数型，禁止含小数点)  

		Max: 300, //一开始启动时，图片的宽和高，有时候图片会很大的，必须要设置一下(整数型，禁止含小数点)，尽可能和scopeWidth值 一致  
	...
	Shear.complete = function(serverdata) { //截图成功完成时，由shearphoto.php返回数据过来的成功包
			if(serverdata.length){
				var first_image = serverdata[0];
				var image_url = relativeUrl + first_image["ImgUrl"];
				$("input#avatar").val(image_url);
				$("img.update-avatar").attr("src", image_url);
			}
			
			this.again();
			this.pointhandle(3e3, 10, "截图完成！已返回！", 2, "#fbeb61", "#3a414c");
	...
	var up = new ShearPhoto.frameUpImg({
		...
		UpType: new Array("jpg", "jpeg", "png"), //图片类限制，上传的一定是图片，你就不要更改了
